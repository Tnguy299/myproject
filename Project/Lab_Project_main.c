...
typedef enum { SHM, CCNM, NEITHER } Climate;     // SHM = Sunny, Hot, Moisture in Atmosphere : CCNM = Cloudy, Cold, Negligible Moisture
Climate current_climate = NEITHER;  // Variable to track the current climate

bool first_run = true;     // Determines if it is, in fact, the first run
...
// Function to update LED state based on environmental readings
void UpdateLedState(uint32_t temp_F, uint32_t light_reading, uint32_t moist_reading)
{
    //If the temperature, light, and moisture readings indicate the following climate conditions
    if (temp_F > 740 && light_reading > 800 && moist_reading > 300)
    {
        // Turn off the RED LED and turn on the GREEN LED
        GPIOPinWrite(GPIO_PORTF_BASE, RED_LED, 0);
        GPIOPinWrite(GPIO_PORTF_BASE, GREEN_LED, GREEN_LED);

        // If the current climate is not SHM, play a sound and update the current climate to SHM
        if (current_climate != SHM) {
            if (!first_run) {
                uprintf("\n%s\n\n\r", "Sunny, Hot, Moisture in Atmosphere\n");
                BzTone(5000, 100); // Play a high pitch sound for 100 ms
            }
            current_climate = SHM;
        }
    }
    // If the temperature, light, and moisture readings indicate CCNM conditions
    else if (temp_F <= 740 && light_reading <= 800 && moist_reading <= 300)
    {
        // Turn off the GREEN LED and turn on the RED LED
        GPIOPinWrite(GPIO_PORTF_BASE, GREEN_LED, 0);
        GPIOPinWrite(GPIO_PORTF_BASE, RED_LED, RED_LED);

        // If the current season is not CCNM, play a sound and update the current season to CCNM
        if (current_climate != CCNM) {
            if (!first_run) {
                uprintf("%s\n\r","Cloudy, Cold, Negligible Moisture\n");
                BzTone(500, 100); // Play a low pitch sound for 100 ms
            }
            current_climate = CCNM;
        }
    }
    // If the readings don't indicate either SHM or CCNM conditions
    else
    {
        // Turn off both the RED and GREEN LEDs
        GPIOPinWrite(GPIO_PORTF_BASE, RED_LED | GREEN_LED, 0);
        current_climate = NEITHER;
    }
...

{
   ...
   // LsInit();
    ...
    uprintf("%s\n\r", "Final Project: Climate Sensor");
